...
learning_rate = 5e-5
l1_reg = 5e-6
optimizer = torch.optim.SGD(net.parameters(), lr=learning_rate, momentum = 0.9, weight_decay = 5e-6)
...
for i in range(num_epoch):
    for j in range(iteration):
        ...
        reg_loss = 0
        for param in net.parameters():
            target = Variable(torch.Tensor(param.size()).zero_()).cuda(2)
    ...
    scheduler.step()
    output = "/home/yiqin/two_path_cnn/phase1_TPWconv_net_withL1" + str(i) + '.txt'
